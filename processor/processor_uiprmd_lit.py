...
class Processor(IO):
                    ...
                    if self.meta_info['is_best']:
                        ...
                        result_dict = dict(
                            zip(self.data_loader['test'].dataset.sample_name,
                                self.result))
                        self.io.save_pkl(result_dict, 'test_result_epoch_{}.pkl'.format(epoch+1))
                        self.io.save_pkl(result_dict, 'best_result.pkl')
                        print('saving result DONE!')
                    if self.meta_info['best_t1'] > 0.999:
                        break
        ...
        # test phase
            ...
            if self.arg.save_result:
                ...
                result_dict = dict(
                    zip(self.data_loader['test'].dataset.sample_name,
                        self.result))
                self.io.save_pkl(result_dict, 'test_result{}.pkl'.format(epoch+1))
                print('saving result DONE!')
    ...
    def get_parser(add_help=False):
        ...
        # visulize and debug
        parser.add_argument('--log_interval', type=int, default=25, help='the interval for printing messages (#iteration)')
