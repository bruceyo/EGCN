...
for group, ids in group_dic.items():
    for id in range(1,ids+1):
        ...
        for i in range(0,26): # range(0,42,2)
            sample_id = i
            if (math.isnan(labels.iat[sample_id+1])):
                #print(str(samples) + " samples of Es4 retrieved!")
                break
            if int(labels.iat[sample_id]) == 0 or int(labels.iat[sample_id+1]) == 0:
                exercise_id = 'E007'
                continue
            if int(labels.iat[sample_id]) == 1 or int(labels.iat[sample_id+1]) == 1:
                exercise_id = 'E008'
                continue
            ...
            sample_frame_counts.append(sample_frame_count)
            if sample_frame_count > 150:
                print(group_id, samples)
#'''
            ...
            save_name = group_label_dic[group] + subject_id + exercise_id + label
            if debug:
                save_path = '/media/bruce/2T/data/KiMoRe/skeleton_debug/' + save_name
            else:
