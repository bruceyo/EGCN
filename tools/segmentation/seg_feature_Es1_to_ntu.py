...
for group, ids in group_dic.items():
    for id in range(1,ids+1):
        ...
        for i in range(0,7): # range(0,42,2)
            sample_id = i
            if(math.isnan(labels.iat[sample_id+1])):
                print(str(i+1) + " samples of action " + group_id + " retrieved!")
                break
